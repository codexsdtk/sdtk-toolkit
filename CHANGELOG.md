...
## [0.5.0] - 2026-07-08
### Added
- Public free mirror now covers the full free suite: `sdtk-wiki` (local Atlas graph viewer + SDTK-WIKI second brain + context pack) and the unified `sdtk-kit` installer (`npm install -g sdtk-kit`), alongside `sdtk-spec`, `sdtk-code`, and `sdtk-ops`.
### Changed
- Refreshed `sdtk-spec`, `sdtk-code`, and `sdtk-ops` sources to the current shipped versions.
- The local Atlas viewer keeps its graph and docs experience in the open-source edition; grounded Ask is a paid capability and is not bundled here.

## [0.4.2] - 2026-04-16
### Added
- `sdtk-spec init` now scaffolds `governance/ai/session/SDTK_ACTIVE_BOOTSTRAP.md` and `governance/ai/session/SDTK_AGENT_WORKING_RULES.md` into the target project so new sessions can load compact bootstrap context automatically.
### Changed
- Reframed generated `AGENTS.md`, `CLAUDE.md`, and `CODEX.md` as SDTK Suite guidance instead of SDTK-SPEC-only guidance.
- Clarified runtime routing so users are directed to `SDTK-CODE` for build/fix implementation work and `SDTK-OPS` for deploy/infra/ops work, while keeping `SDTK-SPEC` as the upstream orchestration layer.
### Fixed
- Corrected the init/runtime guidance gap where generated adapter files described only the SPEC phase workflow and failed to mention the downstream `CODE` and `OPS` product paths.
