...
2. **Harvest.** Scan sessions since the last harvest:
   - **Claude Code:** `~/.claude/projects/<project-slug>/*.jsonl` — extract per-session digests: user intents, tool usage, and correction signals (negative: "vẫn sai", "làm lại", "that's wrong", "I told you", "again"; positive: "perfect", "đúng rồi", "great").
   ...
   - **Filters (both sources):** skip meta prompts (bare slash commands, pasted-text stubs, system noise) and skip any content carrying the `<!-- sdtk-evolve:harvest-session:v1 -->` marker or the legacy `SDTK-EVOLVE CYCLE` marker (self-pollution guard).
...
- It is the operator-approved, locally learned layer that sessions read at start (the CLAUDE.md template carries the pointer). It survives `sdtk update` and re-init because it lives outside every kit-managed file.
