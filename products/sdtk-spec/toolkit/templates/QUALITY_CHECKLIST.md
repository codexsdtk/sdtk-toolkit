...
| 7 | `sdtk-spec config check` passes (no UPDATE_ME placeholders - stack/test commands are real) | [ ] Pending | Dev Agent | Run `sdtk-spec config apply-profile <name>` if unfilled | |
| **8** | **DEV READY FOR SDTK-CODE** | [ ] Pending | **Dev Gate** | **If ready: suggest `sdtk-code start ...`; if blocked: keep blockers visible** | |
