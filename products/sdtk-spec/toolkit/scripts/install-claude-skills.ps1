...
function Get-ClaudeManagedReferenceSources {
  ...
  switch ($SkillName) {
    ...
    'screen-design-spec' { return @(
      ...
      'skills/sdtk-screen-design-spec/references/commerce-profile-screens.md'
    ) }
...
# Strict count assertions. Expected skills derive from the payload source of
# truth (dirs shipping a SKILL.md) so the integrity check still catches copy
# failures without drifting every time a skill is added (BK-316 fix; the old
# hard-coded 14 broke install when the 15th skill landed).
$expectedSkills = @(Get-ChildItem -Path $skillsSource -Directory | Where-Object {
    Test-Path -LiteralPath (Join-Path $_.FullName 'SKILL.md')
  }).Count
$expectedManagedRefs = 12
