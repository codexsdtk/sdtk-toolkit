...
# $IsWindows is an automatic variable on PowerShell 7 (Core); on Windows
# PowerShell 5.1 it does not exist, so a null falls back to Windows behavior.
$script:IsWindowsHost = if ($null -ne $IsWindows) { [bool] $IsWindows } else { $true }

# `/.dockerenv`, an explicit env flag, or the -ExternallySandboxed switch mark a
# container/externally-sandboxed host. Codex's workspace-write sandbox needs
# unprivileged user namespaces that such hosts usually disable, so we swap
# --full-auto for --dangerously-bypass-approvals-and-sandbox there. Read-only
# sandbox does not create namespaces and keeps working, so it is left alone.
function Test-ExternallySandboxedHost([bool] $SwitchRequested) {
  if ($SwitchRequested) { return $true }
  if ($env:SDTK_EXTERNALLY_SANDBOXED -eq '1') { return $true }
...
function Stop-ProcessTree([int] $ProcessId) {
  ...
  # POSIX: enumerate children via `ps --ppid`, recurse, then SIGKILL.
  $childOutput = ''
  try { $childOutput = (& ps -o pid= --ppid $ProcessId 2>$null | Out-String) } catch { $childOutput = '' }
  foreach ($line in ($childOutput -split "`n")) {
    $pidText = $line.Trim()
    if ($pidText -match '^\d+$') { Stop-ProcessTree -ProcessId ([int] $pidText) }
  }
  try { & kill -9 $ProcessId 2>$null } catch { }
}
...
# POSIX single-quote escaping: wrap in '...', and render embedded single quotes
# as the classic '\'' sequence so the shell sees the literal value.
function Convert-ToShQuoted([string] $Value) {
  return "'" + ($Value -replace "'", "'\''") + "'"
}

# Build the codex sandbox flag(s) for a run. Writable runs use --full-auto,
# except on an externally-sandboxed host where that sandbox cannot create
# namespaces — there we bypass Codex's own sandbox (the container is the
# boundary). Read-only runs always use the read-only sandbox.
function Get-CodexSandboxArgs([bool] $UseWorkspaceWrite, [bool] $ExternallySandboxed) {
  if (-not $UseWorkspaceWrite) { return @('--sandbox', 'read-only') }
  if ($ExternallySandboxed) { return @('--dangerously-bypass-approvals-and-sandbox') }
  return @('--full-auto')
}
...
function Invoke-CodexProcessWithWatchdog(
  ...
  [bool] $ExternallySandboxed,
  [int] $WatchdogSeconds
) {
  # Raw (unquoted) argument vector; each element is quoted per-platform below.
  $codexArgs = @(
    'codex', 'exec',
    '-c', 'mcp_servers={}',
  ...
  $codexArgs += (Get-CodexSandboxArgs $UseWorkspaceWrite $ExternallySandboxed)
  if ($ModelName) { $codexArgs += @('--model', $ModelName) }
  $codexArgs += '-'

  $runnerScriptPath = $null
  if ($script:IsWindowsHost) {
    $quoted = $codexArgs | ForEach-Object { Convert-ToCmdQuoted $_ }
    $commandLine = ($quoted -join ' ') + ' < ' + (Convert-ToCmdQuoted $PromptPath)
    if ($LogPath) { $commandLine += ' > ' + (Convert-ToCmdQuoted $LogPath) + ' 2>&1' }
    ...
    # POSIX: write the composed command line to a temp shell script and run it
    # via /bin/sh. Passing the redirect-laden command line straight through
    # Start-Process -ArgumentList mangles the `<`/`>` metacharacters (the child
    # then loses its stdin redirect and Codex aborts with "stdin is not a
    # terminal"); a script file sidesteps all argument re-quoting. No cmd.exe
    # and no -WindowStyle (unsupported on the PowerShell Linux edition).
    $quoted = $codexArgs | ForEach-Object { Convert-ToShQuoted $_ }
    $commandLine = ($quoted -join ' ') + ' < ' + (Convert-ToShQuoted $PromptPath)
    if ($LogPath) { $commandLine += ' > ' + (Convert-ToShQuoted $LogPath) + ' 2>&1' }
    $runnerScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) ("sdtk-mailbox-codex-" + [guid]::NewGuid().ToString('N') + '.sh')
    $embedded = "#!/bin/sh`n" + $commandLine + "`n"
    [System.IO.File]::WriteAllText($runnerScriptPath, $embedded, $Utf8NoBom)
    $process = Start-Process -FilePath '/bin/sh' -ArgumentList @($runnerScriptPath) -WorkingDirectory $RepoRootPath -PassThru
  }
  ...
  try {
    ...
    if ($runnerScriptPath) { Remove-Item $runnerScriptPath -Force -ErrorAction SilentlyContinue }
  }
}
...
$isExternallySandboxed = [bool] (Test-ExternallySandboxedHost ([bool] $ExternallySandboxed))
$cliArgs += (Get-CodexSandboxArgs $useWorkspaceWrite $isExternallySandboxed)
...
if ($DryRun) {
  [pscustomobject]@{
    ...
    ExternallySandboxed = $isExternallySandboxed
    HostIsWindows = $script:IsWindowsHost
...
try {
  $processResult = Invoke-CodexProcessWithWatchdog -RepoRootPath $RepoRoot -PromptPath $promptPath -ReportPath $ReportFile -LogPath $LogFile -ModelName $Model -UseWorkspaceWrite:$useWorkspaceWrite -ExternallySandboxed:$isExternallySandboxed -WatchdogSeconds $watchdogSeconds
