...
- Stop if path naming policy from `governance/ai/core/SDTK_API_PATH_STYLE_POLICY.md` cannot be resolved without a product decision; record as OQ-xx and escalate to `@pm`.
...
2. Load reference rules:
   - `toolkit/templates/docs/api/YAML_CREATION_RULES.md` for YAML contract rules
   - `toolkit/templates/docs/api/API_DESIGN_FLOWCHART_CREATION_RULES.md` for flow list / flowchart rules
...
- All path segments match the canonical naming policy in `governance/ai/core/SDTK_API_PATH_STYLE_POLICY.md`.
...
Do not finalize YAML or ENDPOINTS.md before reading and applying both reference rule files. Generating endpoints without `toolkit/templates/docs/api/YAML_CREATION_RULES.md` and `toolkit/templates/docs/api/API_DESIGN_FLOWCHART_CREATION_RULES.md` produces non-conformant output that will fail downstream review.
...
| Skip flowchart rules and free-style PlantUML | Produces flows that deviate from SDTK conventions | Apply `toolkit/templates/docs/api/API_DESIGN_FLOWCHART_CREATION_RULES.md` before generating any flow |

## Reference Loading Guidance
Read these files at skill start, not on demand:
