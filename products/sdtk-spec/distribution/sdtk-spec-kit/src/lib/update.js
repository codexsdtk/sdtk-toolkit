...
const PACKAGE_NAME = "sdtk-spec-kit";
const CLI_NAME = "sdtk-spec";
const CLI_BIN = path.resolve(__dirname, "..", "..", "bin", "sdtk.js");
...
function buildPlan(options) {
  ...
  if (options.skipProjectFiles) {
    ...
    if (options.skipRuntimeAssets) {
      ...
      plan.runtimeRefreshCommand =
        "skipped (SDTK-SPEC refreshes runtime assets through init, and project file refresh was skipped)";
    }
    return plan;
  }
  ...
  if (options.skipRuntimeAssets) {
    initArgs.push("--skip-runtime-assets");
    plan.runtimeRefreshCommand = "skipped (--skip-runtime-assets)";
  } else {
    plan.runtimeRefreshCommand =
      `${CLI_NAME} init refreshes runtime assets for SDTK-SPEC unless --skip-runtime-assets is set`;
  }
  if (options.verbose) {
...
function parseUpdateOptions(args) {
  ...
  if (runtime && !flags["skip-project-files"]) {
    assertProjectRefreshTargetAllowed("sdtk-spec", "update", projectPath);
  }
...
function printPlan(plan) {
  console.log("SDTK-SPEC update plan");
...
async function applyPlan(plan, options) {
  console.log("");
  console.log("Applying SDTK-SPEC update...");
  ...
  console.log("");
  console.log("SDTK-SPEC update completed successfully.");
  // BK-346: fragmentation is born exactly at update time (BK-322 lesson).
  console.log('Tip: run "sdtk doctor" to confirm every suite CLI now serves the updated copy (detects shadowed installs).');
}
