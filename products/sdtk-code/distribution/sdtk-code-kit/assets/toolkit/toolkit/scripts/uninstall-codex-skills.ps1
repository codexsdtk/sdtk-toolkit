...
if ($managedSkillNames.Count -ne 12) {
  throw "Expected 12 managed SDTK-CODE skills but found $($managedSkillNames.Count)."
}
...
if (-not $Quiet) {
  ...
  if ($Scope -eq 'project') {
    Write-Host "- Project-local Codex uninstall touched only SDTK-CODE-managed assets under <project>/.codex/skills."
  }
}
