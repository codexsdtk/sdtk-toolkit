...
function cmdRuntimeStatus(flags) {
  ...
  for (const scope of scopes) {
    ...
    const status = !exists || !hasManagedSkills
      ? "not installed"
      : allInstalled
        ? `installed (${skillCount}/${managed.length} SDTK-CODE skills)`
        : `partial (${skillCount}/${managed.length} SDTK-CODE skills)`;
...
async function cmdRuntime(args) {
  ...
  if (!subcommand) {
    throw new ValidationError(
      `Missing subcommand. Usage: sdtk-code runtime <${SUBCOMMANDS.join("|")}> [options]`
    );
  }
